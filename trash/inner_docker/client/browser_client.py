...
class BrowserClient:
    def __init__(self):
        ...
        self.context = None
        self.auth_data = {"email": "", "password": "", "verification_code": ""}
        self.auth_status = {
    ...
    async def perform_authentication(self):
        """Выполняет полную аутентификацию с использованием установленных данных"""
        print("🔄 Выполняю аутентификацию...")
        try:
            # Ищем кнопку Log In
            login_selectors = [
    ...
    async def provide_email(self, email: str) -> bool:
        ...
        await self.set_auth_data(email=email)
        return await self._provide_email()

    async def provide_password(self, password: str) -> bool:
        """Предоставляет пароль для пошаговой аутентификации"""
        await self.set_auth_data(password=password)
        return await self._provide_password()

    async def provide_verification_code(self, code: str) -> bool:
        """Предоставляет код подтверждения для пошаговой аутентификации"""
        await self.set_auth_data(verification_code=code)
        return await self._handle_verification_code()
