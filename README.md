...
5. **Авторизация** - использует данные из .env для email и пароля, с возможностью ввода кода подтверждения через API.
6. **Автоматическая авторизация и сохранение сессий** - реализовано сохранение состояния браузера и cookies после успешной авторизации.
7. **Автоматическое восстановление** - автоматически перезапускает систему в случае возникновении ошибок с использованием последнего сохранённого состояния.
...
Поддерживает формат запросов OpenAI API. Извлекает system-промпт и последний user-message, игнорируя остальные поля для совместимости систем.
